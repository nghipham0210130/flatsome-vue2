<template>
  ...
  <div id="loginModal" class="modal" :key="componentKey">
    <button tupe="button" class="modal__icon icon__close" @click="closeModal()">
      X
    </button>
    <div class="modal__content">
      <validation-observer v-slot="{ handleLogin }">
        <h4 class="login__title">Login</h4>
        <!-- Form Login -->
        <form id="loginForm" @submit.prevent="handleLogin(login)">
          <validation-provider
            name="e-mail"
          ...
          >
            ...
            <span class="text__danger">{{ errors[0] }}</span>
      ...
      <hr />
      <validation-observer v-slot="{ handleRegister }">
        <h4 class="register__title">Register</h4>
        <!-- Form Register -->
        <form id="registerForm" @submit.prevent="handleRegister(register)">
          <validation-provider
            name="e-mail"
            rules="required|email"
            v-slot="{ errors }"
          >
            ...
            <input v-model="formRegister.email" type="email" />
          </validation-provider>
          <validation-provider
            name="password"
            rules="required|min:6"
